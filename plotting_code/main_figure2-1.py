...
# Use LaTeX text rendering and Computer Modern to match paper body text
plt.rcParams.update({
    "text.usetex": True,
    "font.family": "serif",
    "font.serif": ["Computer Modern Roman"],
    "font.size": 9,           # matches LaTeX body text
    "axes.labelsize": 9,
    "axes.titlesize": 9,
    "xtick.labelsize": 9,
    "ytick.labelsize": 9,
...
# averages per method
avg = {m: {meth: np.mean(data[m][meth]) for meth in methods} for m in metrics}

# -----------------------------
# 2) Plot (clean, paper-style)
# -----------------------------
fig, axes = plt.subplots(1, 4, figsize=(7.0, 3.5), constrained_layout=True)
...
for i, metric in enumerate(metrics):
    ...
    bars = ax.bar(methods, vals, color=[colors[m] for m in methods], width=0.55)
    ax.set_ylim(0, 1)
    ax.set_title(metric)
    if i == 0:
...
plt.savefig("setup2_method_averages_paper.png", dpi=600, bbox_inches="tight")
plt.savefig("setup2_method_averages_paper.pdf", bbox_inches="tight")