...
#    GSE94820        20      chordal   10            50      kmeans      11       power1.6   5        20
  ...
  #  GSE94820       20      chordal   10            50      spectral        11       power1.6   5        20
#  GSE67835       20      geodesic   10            50      spectral        11       power1.6   5        20
#  GSE75748cell       20      geodesic   10            50      spectral        11       power1.6   5        20
#    GSE109979_329cell       20      geodesic   10            50      spectral        11       power1.6   5        20
#  GSE84133human4      50      geodesic   19            100      spectral        19       power1.6   5        50

 GSE75748time       100      chordal  23           200      kmeans       25      power2   5       100
 GSE94820       100     chordal  23           200     kmeans       25      power2  5        100
 GSE67835       100     chordal   23           200      kmeans        25       power2  5        100
 GSE75748cell       100      chordal   23          200      kmeans        25       power2   5        100
   GSE109979_329cell       100     chordal  23            200    kmeans        25       power2   5        100
#  GSE84133human4    100     chordal   23           200      spectral        25       power2   5        100
#  GSE84133human4     100    geodesic   23           200      spectral       25      power2   5        100


#  GSE84133human1      100      chordal   23            200      spectral        25       power2   5        100
...
def classical_mds_from_dist(D, target_dim=20):
    ...
    Returns an embedding Z (n  \times target_dim).
    """
    n = D.shape[0]
...
def baseline_umap_compare(X_pca, y, scales, out_dim, random_state, results_dir, tag_prefix,
    ...
    for nn in scales:
        ...
        print(f"  nn={nn:>3d}  ACC={acc:.4f}  ARI={ari:.4f}  NMI={nmi:.4f}  Purity={pur:.4f}")
        rows.append((nn, acc, ari, nmi, pur))

    arr = np.array([[a,b,c,d] for (_,a,b,c,d) in rows], dtype=float)
    acc_m, ari_m, nmi_m, pur_m = arr.mean(axis=0)
    print(f"\n[Baseline Avg] over nn={list(scales)}  "
          f"ACC={acc_m:.4f}  ARI={ari_m:.4f}  NMI={nmi_m:.4f}  Purity={pur_m:.4f}\n")

    os.makedirs(results_dir, exist_ok=True)
    csv_path = os.path.join(results_dir, f"{tag_prefix}_baseline_pca_umap{out_dim}d_{cluster_method}.csv")
    with open(csv_path, "w") as f:
        f.write("nn,ACC,ARI,NMI,Purity\n")
        for nn, acc, ari, nmi, pur in rows:
            f.write(f"{nn},{acc:.6f},{ari:.6f},{nmi:.6f},{pur:.6f}\n")
        f.write(f"avg,{acc_m:.6f},{ari_m:.6f},{nmi_m:.6f},{pur_m:.6f}\n")
    ...
    payload = {
        ...
        "per_scale": [{"nn": int(nn), "ACC": a, "ARI": r, "NMI": n, "Purity": p} for (nn, a, r, n, p) in rows],
        "average": {"ACC": acc_m, "ARI": ari_m, "NMI": nmi_m, "Purity": pur_m},
    }
    save_json(payload, json_path)
    return {"ACC": acc_m, "ARI": ari_m, "NMI": nmi_m, "Purity": pur_m}
...
def baseline_pca_compare(X_pca, y, results_dir, tag_prefix, cluster_method="spectral", gamma=1.0, random_state=RANDOM_STATE):
    ...
    nmi = normalized_mutual_info_score(y, y_pred)
    pur = purity_score(y, y_pred)
    print(f"[Baseline-PCA@{Z.shape[1]}-{cluster_method}] ACC={acc:.4f}  ARI={ari:.4f}  NMI={nmi:.4f}  Purity={pur:.4f}")

    os.makedirs(results_dir, exist_ok=True)
    with open(os.path.join(results_dir, f"{tag_prefix}_baseline_pca_only_{cluster_method}.csv"), "w") as f:
        f.write("dim,ACC,ARI,NMI,Purity\n")
        f.write(f"{Z.shape[1]},{acc:.6f},{ari:.6f},{nmi:.6f},{pur:.6f}\n")
    save_json({"dim": int(Z.shape[1]), "ACC": acc, "ARI": ari, "NMI": nmi, "Purity": pur, "cluster_method": cluster_method},
              os.path.join(results_dir, f"{tag_prefix}_baseline_pca_only_{cluster_method}.json"))
    return {"ACC": acc, "ARI": ari, "NMI": nmi, "Purity": pur}
...
def baseline_pca_to_outdim_compare(X_pca, y, out_dim, results_dir, tag_prefix,
    ...
    nmi = normalized_mutual_info_score(y, y_pred)
    pur = purity_score(y, y_pred)
    print(f"[Baseline-PCA→PCA{out_dim}-{cluster_method}] ACC={acc:.4f}  ARI={ari:.4f}  NMI={nmi:.4f}  Purity={pur:.4f}")

    os.makedirs(results_dir, exist_ok=True)
    with open(os.path.join(results_dir, f"{tag_prefix}_baseline_pca_to_{out_dim}_{cluster_method}.csv"), "w") as f:
        f.write("dim,ACC,ARI,NMI,Purity\n")
        f.write(f"{out_dim},{acc:.6f},{ari:.6f},{nmi:.6f},{pur:.6f}\n")
    save_json({"dim": int(out_dim), "ACC": acc, "ARI": ari, "NMI": nmi, "Purity": pur, "cluster_method": cluster_method},
...
def baseline_pca_direct_to_outdim_compare(X, y, out_dim, results_dir, tag_prefix,
    ...
    nmi = normalized_mutual_info_score(y, y_pred)
    pur = purity_score(y, y_pred)
    print(f"[Baseline-RAW-PCA→{out_dim}-{cluster_method}] ACC={acc:.4f}  ARI={ari:.4f}  NMI={nmi:.4f}  Purity={pur:.4f}")
    ...
    save_json(
        {"dim": int(out_dim), "ACC": acc, "ARI": ari, "NMI": nmi, "Purity": pur, "cluster_method": cluster_method},
...
def run_one(data_name, out_dim, distance, subspace_dim,
    ...
    g_ari = float(adjusted_rand_score(y, labels))
    g_nmi = float(normalized_mutual_info_score(y, labels))
    g_pur = purity_score(y, labels)
    ...
    # Grassmann
    row["G_ACC"] = g_acc; row["G_ARI"] = g_ari; row["G_NMI"] = g_nmi; row["G_Purity"] = g_pur
    # UMAP (avg across scales)
    row["B_ACC"] = float(b_umap["ACC"])
    row["B_ARI"] = float(b_umap["ARI"])
    row["B_NMI"] = float(b_umap["NMI"])
    row["B_Purity"] = float(b_umap["Purity"])
    # PCA@pca_dim
    row["PCA_ACC"] = float(b_pca["ACC"])
    row["PCA_ARI"] = float(b_pca["ARI"])
    row["PCA_NMI"] = float(b_pca["NMI"])
    row["PCA_Purity"] = float(b_pca["Purity"])
    # PCA→PCA(out_dim)
    row["PCAout_ACC"] = float(b_pca_out["ACC"])
    row["PCAout_ARI"] = float(b_pca_out["ARI"])
    row["PCAout_NMI"] = float(b_pca_out["NMI"])
    row["PCAout_Purity"] = float(b_pca_out["Purity"])
    # RAW→PCA(out_dim) [NEW]
    row["PCAraw_ACC"] = float(b_pca_raw_out["ACC"])
    row["PCAraw_ARI"] = float(b_pca_raw_out["ARI"])
    row["PCAraw_NMI"] = float(b_pca_raw_out["NMI"])
    row["PCAraw_Purity"] = float(b_pca_raw_out["Purity"])
...
def main():
    ...
    for cfg in run_cfgs:
        ...
        metric_keys = [
            ...
            "PCAout_ACC","PCAout_ARI","PCAout_NMI","PCAout_Purity",
            "PCAraw_ACC","PCAraw_ARI","PCAraw_NMI","PCAraw_Purity"
        ]
